...
$(document).ready(function() {
  ...
  $(document).on("click", ".save-btn", function() {
      ...
      let existingTasks = localStorage.getItem('taskData');
      existingTasks = existingTasks ? JSON.parse(existingTasks) : [];

      // Add the new task to the array
      existingTasks.push(taskText);

      // Store the updated task array in local storage
      localStorage.setItem('taskData', JSON.stringify(existingTasks));

      alert("Task saved to local storage!");
  });
});
// running out of time and seem to have a stringify issue that is preventing the events being readded on page refresh. Even tried using try blocks